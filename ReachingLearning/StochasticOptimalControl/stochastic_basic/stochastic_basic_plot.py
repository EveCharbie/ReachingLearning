...
def plot_controls(variable_data, socp_basic, save_path_socp_basic):
    ...
    # Optimization variables
    axs[0].plot(time_vector, variable_data["tau_opt"][0, :], ".", markersize=1, color=SOCP_BASIC_color)
    axs[1].plot(time_vector, variable_data["tau_opt"][1, :], ".", markersize=1, color=SOCP_BASIC_color)
    ...
    plt.tight_layout()
    save_path_fig = save_path_socp_basic.replace(".pkl", "_plot_taus.png").replace("/results/", "/figures/")
    plt.savefig(save_path_fig)
    # plt.close()
    ...
    plt.tight_layout()
    save_path_fig = save_path_socp_basic.replace(".pkl", "_plot_muscles.png").replace("/results/", "/figures/")
    plt.savefig(save_path_fig)
    plt.show()
    # plt.close()
...
def plot_hand_trajectories(variable_data, ocp_multimodel, n_simulations, save_path_ocp_multimodel):
    ...
    n_motor_noises = ocp_multimodel["model"].nb_q * n_random
    n_sensory_noises = 4 * n_random
    n_noises = n_motor_noises + n_sensory_noises
