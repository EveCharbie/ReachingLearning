...
def get_variables_from_vector_torque_driven(n_q, n_shooting, vector):
...
def get_states_and_controls_torque_driven(
    n_q,
    n_shooting,
    q_opt,
    qdot_opt,
    ...
    for i_node in range(n_shooting + 1):
        ...
        if i_node < n_shooting:
            u_opt[:n_q, i_node] = tau_opt[:, i_node].flatten()
            tau_offset = n_q

    return x_opt, u_opt

def save_ocp_torque_driven(
    w_opt: np.ndarray,
    ocp: dict[str, any],
    save_path_ocp: str,
    tol: float,
    ...
    # Get optimization variables
    q_opt, qdot_opt, tau_opt = get_variables_from_vector_torque_driven(
        n_q, n_shooting, w_opt
    )
    q0, qdot0, tau0 = get_variables_from_vector_torque_driven(
        n_q, n_shooting, w0
    )
    lbq, lbqdot, lbtau = get_variables_from_vector_torque_driven(
        n_q, n_shooting, lbw
    )
    ubq, ubqdot, ubtau = get_variables_from_vector_torque_driven(
        n_q, n_shooting, ubw
    )

    x_opt, u_opt = get_states_and_controls_torque_driven(
        n_q,
        n_shooting,
        q_opt,
        qdot_opt,
    ...
    # Reintegrate the solution
    x_integrated = integrate_single_shooting(ocp, np.vstack((q_opt, qdot_opt)), tau_opt)
    q_integrated = x_integrated[0:2, :]
    qdot_integrated = x_integrated[2:4, :]
